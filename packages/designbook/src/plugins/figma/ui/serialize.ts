...
import { parseCssColor, type Rgba } from "@designbookapp/designbook/config";
import {
  blockStackGap,
  decideTextSizing,
  flexToLayout,
...
/**
 * Token-attribution rows for the serializer (G2a): each CSS var (or derived
 * `cssValue` expression) is probed live and concrete colors/dimensions are
 * attributed back to `figmaName`. Passed EXPLICITLY by the caller (the figma
 * plugin computes it from the neutral TokenSource registry + its naming
 * options) — there is no global figma token source anymore.
 */
type SerializeToken = {
  /** CSS custom property name WITHOUT the leading `--`. */
  cssVar: string;
  /** Figma variable name (token name through the plugin's NameMap). */
  figmaName: string;
  type: "color" | "dimension" | "number" | "string";
  /**
   * Raw CSS expression to probe INSTEAD of `var(--cssVar)` when the custom
   * property may not exist in the document (derived tokens like the Tailwind
   * radius scale).
   */
  cssValue?: string;
};
...
/**
 * Probes the caller-provided token rows (see `SerializeOptions.tokens`)
 * against the live canvas theme element: each color token's CSS var is
 * resolved to its computed rgb inside a hidden probe, each dimension token to
 * px. Collisions keep the FIRST token in list order.
 */
function buildTokenMaps(rootEl: Element, tokens: SerializeToken[]): TokenMaps {
...
async function serializeComponent(
  ...
  const ctx: WalkContext = {
    scaleInv,
    tokens: buildTokenMaps(rootEl, opts.tokens ?? []),
    warnings,
    images: {},
...
export type { SerializeOptions, SerializeResult, SerializeToken };
