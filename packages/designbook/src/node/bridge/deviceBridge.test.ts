...
import { createDeviceBridge } from "./deviceBridge.ts";
...
describe("deviceBridge", () => {
  it("rejects invoke when no device is connected", async () => {
    const bridge = createDeviceBridge("figma");
    expect(bridge.isConnected()).toBe(false);
    await expect(bridge.invoke("figma_get_selection", {})).rejects.toThrow(
      /no figma device connected/i,
    );
  });
  ...
  it("rejects the pending invoke when the device returns ok:false", async () => {
    const bridge = createDeviceBridge("figma");
    const { socket, sent, emitMessage } = createMockSocket();
    bridge.attachSocket(socket);
    emitMessage({ type: "hello", protocol: 1 });
