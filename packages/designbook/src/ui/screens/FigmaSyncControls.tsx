 ...
 * inside the left-rail Figma tab (FigmaPanel). Push serializes the live canvas
 * preview (see figmaSerialize.ts) and POSTs the resulting `RenderTree` to
 * `/api/figma/push`, which relays it to the connected Figma plugin. Pull POSTs
 * to `/api/figma/pull` (declarative read-back → annotated HTML) and drafts the
 * Pi handoff prompt straight into the chat tab — nothing is sent; the user's
 * send click in the chat IS the confirm gate. Both buttons are enabled only while a
 * plugin is connected — the connection state is polled by the surrounding
 * FigmaPanel (`/api/figma/status`) and handed in as a prop.
 */
...
import { getAdapterRuntime, getFigmaTokenSource } from "@designbook-ui/adapterRuntime";
import { apiUrl } from "@designbook-ui/designbook";
import { serializeComponent } from "@designbook-ui/previewHost";
import { formatPullPrompt, type PullRenderContext } from "@designbookapp/designbook/config";
...
function FigmaSyncControls({
  ...
  async function pull() {
    ...
    try {
      const response = await fetch(apiUrl("/api/figma/pull"), {
        method: "POST",
      ...
      onAddToChat(formatPullPrompt({ componentId: entryId, sourcePath, html, render }));
    ...
    try {
      ...
      const { tree, warnings } = await serializeComponent(rootEl, {
        ...
        meta: {
          ...
          collection: getFigmaTokenSource()?.collection,
          dimensions: otherDimensionValues(),
        },
      });

      const response = await fetch(apiUrl("/api/figma/push"), {
        method: "POST",
        headers: { "content-type": "application/json" },
